...
exports.DEFAULT_CONNECTED_APP_INFO = {
  ...
  legacyClientId: "PlatformCLI", // <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< ELTOROIT: CHANGED THIS!
...
class AuthInfo extends kit_1.AsyncOptionalCreatable {
  ...
  static getDefaultInstanceUrl() {
    const configuredInstanceUrl = configAggregator_1.ConfigAggregator.getValue(
      orgConfigProperties_1.OrgConfigProperties.ORG_INSTANCE_URL
    ).value;
  ...
  static async listAllAuthorizations(orgAuthFilter = (orgAuth) => !!orgAuth) {
    const stateAggregator =
      await stateAggregator_1.StateAggregator.getInstance();
    const config = (
    ...
    for (const org of orgs) {
      ...
      try {
        ...
        const { orgId, instanceUrl, devHubUsername, expirationDate, isDevHub } =
          authInfo.getFields();
        final.push({
          aliases,
          configs,
          ...
          isExpired:
            Boolean(devHubUsername) && expirationDate
              ? new Date(
                  (0, ts_types_1.ensureString)(expirationDate)
  ...
  static async hasAuthentications() {
    try {
      ...
      const error = err;
      if (
        error.name === "OrgDataNotAvailableError" ||
        error.code === "ENOENT"
      ) {
        return false;
      }
  ...
  static getAuthorizationUrl(options, oauth2) {
    ...
    const params = {
      ...
      scope:
        options.scope ??
        kit_1.env.getString("SFDX_AUTH_SCOPES", "refresh_token api web"),
    };
  ...
  static async identifyPossibleScratchOrgs(fields, orgAuthInfo) {
    ...
    const logger = await logger_1.Logger.child("Common", {
      tag: "identifyPossibleScratchOrgs",
    });
    ...
    await Promise.all(
      hubAuthInfos.map(async (hubAuthInfo) => {
        try {
          const data = await AuthInfo.queryScratchOrg(
            hubAuthInfo.username,
            fields.orgId
          );
          if (data.totalSize > 0) {
            // if any return a result
            logger.debug(
              `found orgId ${fields.orgId} in devhub ${hubAuthInfo.username}`
            );
            try {
              ...
              logger.debug(
                `set ${
                  hubAuthInfo.username
                } as devhub for scratch org ${orgAuthInfo.getUsername()}`
              );
            } catch (error) {
              logger.debug(
                `error updating auth file for ${orgAuthInfo.getUsername()}`,
                error
              );
            }
          }
          ...
          logger.error(
            `Error connecting to devhub ${hubAuthInfo.username}`,
            error
          );
        }
      })
  ...
  static async getDevHubAuthInfos() {
    return AuthInfo.listAllAuthorizations(
      (possibleHub) => possibleHub?.isDevHub ?? false
    );
  }
  static async queryScratchOrg(devHubUsername, scratchOrgId) {
    const devHubOrg = await org_1.Org.create({
      aliasOrUsername: devHubUsername,
    });
    const conn = devHubOrg.getConnection();
    const data = await conn.query(
      `select Id from ScratchOrgInfo where ScratchOrg = '${sfdc_1.sfdc.trimTo15(
        scratchOrgId
      )}'`
    );
    return data;
  ...
  async save(authData) {
    ...
    if (sfdc_1.sfdc.matchesAccessToken(username)) {
      this.logger.debug(
        "Username is an accesstoken. Skip saving authinfo to disk."
      );
      return this;
    }
  ...
  getClientId() {
    return (
      this.getFields()?.clientId ??
      exports.DEFAULT_CONNECTED_APP_INFO.legacyClientId
    );
  }
  getRedirectUri() {
  ...
  getSfdxAuthUrl() {
    ...
    const instanceUrl = (0, ts_types_1.ensure)(
      decryptedFields.instanceUrl,
    ...
    sfdxAuthUrl += `${(0, ts_types_1.ensure)(
      decryptedFields.refreshToken,
  ...
  async setAsDefault(options = { org: true }) {
    ...
    if (options.devHub) {
      config.set(
        orgConfigProperties_1.OrgConfigProperties.TARGET_DEV_HUB,
        value
      );
    }
    await config.write();
  ...
  async init() {
    ...
    if (
      (0, ts_types_1.isString)(oauthUsername) &&
      sfdc_1.sfdc.matchesAccessToken(oauthUsername)
    ) {
    ...
    // If a username with NO oauth options, ensure authorization already exist.
      ...
      username &&
      !authOptions &&
      !(await this.stateAggregator.orgs.exists(username))
    ) {
    ...
    if (options) {
      ...
      if (this.isTokenOptions(options)) {
        ...
        const userInfo = await this.retrieveUserInfo(
          (0, ts_types_1.ensureString)(options.instanceUrl),
          (0, ts_types_1.ensureString)(options.accessToken)
        );
        this.update({
        ...
        if (this.options.parentUsername) {
          const parentFields = await this.loadDecryptedAuthFromConfig(
            this.options.parentUsername
          );
          ...
          if (process.env.SFDX_CLIENT_SECRET) {
            ...
            Object.assign(options, {
              ...
              privateKey: parentFields.privateKey
                ? (0, path_1.resolve)(parentFields.privateKey)
                : parentFields.privateKey,
            });
          }
        }
        // jwt flow
      ...
      authConfig.isDevHub = await this.determineIfDevHub(
        (0, ts_types_1.ensureString)(authConfig.instanceUrl),
        (0, ts_types_1.ensureString)(authConfig.accessToken)
      );
  ...
  async authJwt(options) {
    ...
    const privateKeyContents = await this.readJwtKey(
      (0, ts_types_1.ensureString)(options.privateKey)
    );
    const { loginUrl = sfdcUrl_1.SfdcUrl.PRODUCTION } = options;
    const url = new sfdcUrl_1.SfdcUrl(loginUrl);
    const createdOrgInstance = (this.getFields().createdOrgInstance ?? "")
      .trim()
      .toLowerCase();
    ...
    const loginAndAudienceUrls = (0, sfdcUrl_1.getLoginAudienceCombos)(
      audienceUrl,
      loginUrl
    );
    for (const [login, audience] of loginAndAudienceUrls) {
      try {
        ...
        // eslint-disable-next-line no-await-in-loop
        authFieldsBuilder = await this.tryJwtAuth(
          options.clientId,
          login,
          audience,
          privateKeyContents
    ...
    if (!authFieldsBuilder) {
      ...
      throw new sfError_1.SfError(
        messages.getMessage("jwtAuthErrors", [authErrors.join("\n")]),
        "JwtAuthError"
      );
    }
    const authFields = {
      accessToken: (0, ts_types_1.asString)(authFieldsBuilder.access_token),
      orgId: parseIdUrl((0, ts_types_1.ensureString)(authFieldsBuilder.id))
        .orgId,
      loginUrl: options.loginUrl,
      privateKey: options.privateKey,
      clientId: options.clientId,
    };
    const instanceUrl = (0, ts_types_1.ensureString)(
  ...
  async buildRefreshTokenConfig(options) {
    ...
    if (!options.clientId) {
      ...
      options.clientSecret =
        exports.DEFAULT_CONNECTED_APP_INFO.legacyClientSecret;
    }
    ...
    let authFieldsBuilder;
    try {
      authFieldsBuilder = await oauth2.refreshToken(
        (0, ts_types_1.ensure)(options.refreshToken)
      );
    } catch (err) {
  ...
  async exchangeToken(options, oauth2 = new jsforce_1.OAuth2(options)) {
    ...
    try {
      this.logger.info(
        `Exchanging auth code for access token using loginUrl: ${options.loginUrl}`
      );
      authFields = await oauth2.requestToken(
        (0, ts_types_1.ensure)(options.authCode)
      );
    } catch (err) {
  ...
  async retrieveUserInfo(instanceUrl, accessToken) {
    ...
    const headers = Object.assign(
      ...
      connection_1.SFDX_HTTP_HEADERS
    );
    try {
      this.logger.info(
        `Sending request for Username after successful auth code exchange to URL: ${userInfoUrl}`
      );
      let response = await new transport_1.default().httpRequest({
        url: userInfoUrl,
        method: "GET",
        headers,
      });
      if (response.statusCode >= 400) {
        ...
        const url = `${baseUrl.toString()}/services/data/${apiVersion}/sobjects/User/${
          userInfoJson.user_id
        }`;
        this.logger.info(
          `Sending request for User SObject after successful auth code exchange to URL: ${url}`
        );
        response = await new transport_1.default().httpRequest({
          url,
          method: "GET",
          headers,
        });
        if (response.statusCode >= 400) {
          ...
          userInfoJson.preferred_username = (0, kit_1.parseJsonMap)(
            response.body
          ).Username;
        }
        return {
          username: userInfoJson.preferred_username,
          organizationId: userInfoJson.organization_id,
        };
      }
    } catch (err) {
      throw messages.createError("authCodeUsernameRetrievalError", [
        err.message,
      ]);
    }
  }
  ...
  throwUserGetException(response) {
    ...
    try {
      ...
      if ((0, ts_types_1.isArray)(body)) {
        errorMsg = body
          .map((line) => line.message ?? line.errorCode ?? "UNKNOWN")
          .join(os.EOL);
      } else {
  ...
  async determineIfDevHub(instanceUrl, accessToken) {
    ...
    try {
      const res = await new transport_1.default().httpRequest({
        url: scratchOrgInfoUrl,
        method: "GET",
        headers,
      });
